...
### Design Philosophy
The UI emphasizes a premium, calm aesthetic with deep slate backgrounds, teal/emerald primary, and purple accent colors. Typography uses Inter for body text and JetBrains Mono for monospace. Visuals incorporate subtle glassmorphism, toned-down gradients, and soft shadows. Components utilize shadcn/ui, including Skeleton loaders. Home page sections (Holdings, Watchlist, MyTokens) are borderless — no outer card borders, using spacing alone to separate sections. Spacing follows an 8px scale with generous padding and section gaps. Visual hierarchy prioritizes the USD balance as the hero element, de-emphasizes secondary labels (Solana, Mainnet badges removed). Action buttons are Phantom-style (60×60px rounded-2xl), with Send as the primary solid-green action. Chart height is 320px. TradingViewModal uses a price-hero layout with the chart taking center stage.
